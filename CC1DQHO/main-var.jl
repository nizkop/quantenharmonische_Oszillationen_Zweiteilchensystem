...
for itec = 1:nbtec
    ...
    for isys = 1:nbsys
		...
		#	println("leer", integrand) 
		    ...
		    if integrand[k] == NaN || integrand[k] == Inf
                                println("hvalues " ,  hvalues[k][i], hvalues[k][j] , "potval ", potval[a]," k ", k , " i ", i, " j ", j) 
                    end
                end
		#for a in 1:length(integrand)
                #        if integrand[a] == NaN || integrand[a] == Inf
                #                println("hvalues " ,  hvalues[a], "potval ", potval[a]," k = a ", a)
                #        end
		end
		#println("end") 
